class DetailView : Gtk.Box {
	...
	private Gtk.Box reviews_container;
	...
	public DetailView(Movie _movie, MovieItem.play_func _play, MovieItem.info_func _info, back_func _back_cb) {
		...
		*/
		...
		this.reviews_container = new Gtk.Box(Gtk.Orientation.VERTICAL, 0);
		...
		if (color.parse("#fff"))
			print("Parsed color just fine\n");
			
		event_box.override_background_color(Gtk.StateFlags.NORMAL, color);
		*/
